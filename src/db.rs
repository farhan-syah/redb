        ...
        match self {
            ...
            Self::Write {
                ...
                if let Some(mem) = tracker.end_write_transaction(*transaction_id) {
                    // The Database was dropped while this transaction was live, deferring the
                    // close to the end of this transaction. The write slot was released above
                    // and no Database handle exists anymore, so the shutdown transaction inside
                    // close_database() cannot block
...
impl Database {
    ...
    pub(crate) fn begin_write_with_allocation_policy(
        ...
        begin_write_with_allocation_policy(&self.transaction_tracker, &self.mem, allocation_policy)
    }
}
...
// require a repair, and closes the storage backend. Called from Database::drop, or from the
// end of the write transaction that was live when the Database was dropped
...
impl Drop for Database {
    fn drop(&mut self) {
        ...
        {
            // The transaction holds everything it needs to operate, so it remains fully
